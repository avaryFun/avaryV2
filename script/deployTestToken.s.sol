...
contract DeployTestToken is Script {
    ...
    function run() external {
        ...
        // First, we need to enable USDC as a paired token
        TokenFactory factory = TokenFactory(TOKEN_FACTORY);
        
        // Enable USDC as a paired token
        try factory.toggleAllowedPairedToken(USDC, true) {
            console.log("Enabled USDC as paired token");
        } catch Error(string memory reason) {
            console.log("Failed to enable USDC as paired token:", reason);
        }
